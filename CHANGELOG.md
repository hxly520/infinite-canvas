...
+ [调整] OpenAI 兼容生图只保留同步/异步接口模式并默认同步；同步失败不再自动改为异步重提，手动异步只创建一次任务并轮询结果，避免重复生成和重复扣费。
+ [修复] 渠道连接信息变化后清空旧模型列表并重新拉取，动态模型改名不再沿用旧 Key 与旧模型组合。
+ [优化] `image.52token.org` 专用于工作台和生成 API，生成后的图片与视频下载地址统一解析到 `video.52token.org` 媒体代理；图片下载后立即保存到浏览器 IndexedDB 并使用本地 Blob 预览。
+ [新增] 生图工作台支持批量生成并发限制，默认并发 5，最高 10。
+ [新增] 视频工作台和画布适配 Seedance、Grok、Omni、Sora、Veo 中转协议，支持长任务继续查询和创建幂等保护。
+ [优化] 视频结果仅通过 52Token 中转或同域边缘地址下载，错误信息隐藏外部媒体地址。
+ [新增] 新增Codex App插件支持。
