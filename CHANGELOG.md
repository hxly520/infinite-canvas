...
+ [修复] 工作台版本检查改为读取 52Token 私有兼容主线，插件注册表和 Canvas Agent 官方更新源保持不变。
+ [新增] 提示词来源新增 Freestylefly GPT Image 2 内置来源。
+ [修复] 升级后继续保留 Xianyu GPT Image 2 动态提示词来源，并按新版来源格式过滤空内容、无图和重复条目。
+ [调整] 升级上游基线到 v0.16.0，保留 52Token 默认渠道、同步/异步生图、媒体代理、本地持久化和统一视频协议适配。
+ [兼容] 上游 v0.16.0 移除的 Ark / Seedance 专用入口仅视为官方变更；52Token 兼容版继续保留旧任务协议，并优先使用统一 `/v1/videos` 协议。
+ [修复] 迁移旧版批量图片画布节点到 v0.15.1 单节点多图片结构，避免升级后历史批量图关系丢失。
+ [调整] 视频模型统一使用 OpenAI 兼容 JSON `/v1/videos` 创建、查询和内容下载协议；参考图片和视频统一使用 `reference_image_urls`、`reference_videos` 字段，保留模型专属的首尾帧约束。
+ [新增] 增加 `minimax-h3-2k` 视频模型适配，固定 2K 输出、5-15 秒时长、5 张图片和 3 段音频参考限制，并支持 5 秒轮询恢复。
+ [优化] 备用异步生图轮询调整为每 5 秒查询、最长等待 30 分钟，并保持 52Token Image2 主生图调用链不变。
