...
- 私有部署中 `image.52token.org` 是图片和视频生成请求的 OpenAI 兼容端点；`video.52token.org` 只代理 `/v1/image-content/*` 与 `/v1/video-content/*` 媒体内容，不得把生成任务提交到媒体代理域名。
- 官方升级只合并正式稳定 Tag，不直接以 `upstream/main` 作为发布基线。遇到官方同类功能时以官方实现为主并兼容保留私有 Image2/Gemini 渠道、媒体代理、同步/异步任务、Seedance/Grok/Omni/Sora/Veo 适配、任务幂等和刷新恢复等二开能力；升级后必须按二开文档与回归清单逐项验证。
