...
export function buildGenerationConfig(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode): AiConfig {
    return {
        ...
        videoReferenceMode: node?.metadata?.referenceMode === "frames" ? "frames" : config.videoReferenceMode || defaultConfig.videoReferenceMode,
        ...
        node.metadata?.status === "loading"
            ? {
                  ...
                  metadata: {
                      ...
                      errorDetails: i18n.t(node.metadata.videoTask ? "canvas.generation.videoPollingPaused" : "canvas.generation.interrupted"),
