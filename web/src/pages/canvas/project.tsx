...
import { createVideoGenerationIdempotencyKey, requestVideoGeneration, storeGeneratedVideo, VideoGenerationTerminalError } from "@/services/api/video";
...
function InfiniteCanvasPage() {
            ...
            prev.map((node) =>
                affectedNodeIds.has(node.id) && node.metadata?.status === NODE_STATUS_LOADING
                    ? node.metadata.videoTask
                        ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, errorDetails: "查询已暂停，点击重试会继续查询原视频任务。" } }
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                ...
                if (mode === "video") {
                    ...
                    const videoIdempotencyKey = createVideoGenerationIdempotencyKey();
                    ...
                    try {
                        const video = await storeGeneratedVideo(
                            await requestVideoGeneration(generationConfig, effectivePrompt, generationContext.referenceImages, generationContext.referenceVideos, generationContext.referenceAudios, {
                                signal: controller.signal,
                                idempotencyKey: videoIdempotencyKey,
                                onTaskChange: (videoTask) => setNodes((prev) => prev.map((node) => (node.id === videoId ? { ...node, metadata: { ...node.metadata, videoTask } } : node))),
                            }),
                        );
                            ...
                            ? node.id === nodeId && !markSourceStatus
                                ...
                                : {
                                      ...
                                      metadata: {
                                          ...
                                          errorDetails,
                                          ...(canCreateReplacement && node.type === CanvasNodeType.Video ? { videoTask: undefined, videoIdempotencyKey: undefined } : {}),
                                      },
                                  }
                            : node,
                    ),
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            const videoIdempotencyKey = node.type === CanvasNodeType.Video ? node.metadata?.videoIdempotencyKey || createVideoGenerationIdempotencyKey() : undefined;
            setRunningNodeId(node.id);
            setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_LOADING, errorDetails: undefined, ...(videoIdempotencyKey ? { videoIdempotencyKey } : {}) } } : item)));
                ...
                if (node.type === CanvasNodeType.Video) {
                    const video = await storeGeneratedVideo(
                        await requestVideoGeneration(generationConfig, prompt, retryImages, context?.referenceVideos || [], context?.referenceAudios || [], {
                            signal: controller.signal,
                            task: node.metadata?.videoTask,
                            idempotencyKey: videoIdempotencyKey,
                            onTaskChange: (videoTask) => setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, videoTask } } : item))),
                        }),
                    );
                        ...
                        item.id === node.id
                            ? {
                                  ...
                                  metadata: {
                                      ...
                                      status: NODE_STATUS_ERROR,
                                      errorDetails,
                                      ...(canCreateReplacement && item.type === CanvasNodeType.Video ? { videoTask: undefined, videoIdempotencyKey: undefined } : {}),
                                  },
                              }
                            : item,
                    ),
        ...
        node.metadata?.status === "loading"
            ? {
                  ...
                  metadata: {
                      ...
                      errorDetails: node.metadata.videoTask ? "页面刷新后查询已暂停，点击重试会继续查询原任务，不会重复创建或扣费。" : "页面刷新后生成已中断，请重新生成。",
                  },
              }
            : node,
    );
